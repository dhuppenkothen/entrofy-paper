...
%In practice, scenarios involving cohort selection are often more complex than the recommendations above can encompass. Below, we introduce some potential use cases that might commonly be encountered and suggest possible strategies for cohort selection.

%\subsubsection*{Cohort Selection with Stand-Out Candidates}

%In certain cases, setting a single threshold during the first merit selection stage may not adequately reflect the reality of the selection problem. For example, there might be stand-out candidates whose qualifications so obviously exceed those of the candidate pool overall that they should definitely be accepted. 

%Application forms can only probe a limited amount of knowledge about a given candidate. There might be candidates whose exact combination of skills or traits might not be fully captured by the selection process, but is so valuable to the workshop that their presence should be guaranteed (for example, Astro Hack Week admits three to four candidates we know to be excellent group facilitators and community builders). Instead of establishing a single threshold above which all candidates are considered of equal merit, one may instead use the results of the merit selection in more complex ways.

%For example, given a grading procedure with a scoring system, organizers may decide that all candidates who receive top scores from all reviewers are automatically accepted. This requires scoring categories and rubrics to be well-calibrated: if the majority of available slots are filled with candidates preselected in this way, there is little scope for a balanced selection, and the rubrics should be reassessed in terms of whether they truly reflect the range of skills present in the candidates or whether a more fine-grained approach might be advisable. 

%Similarly, one may encourage organizers to flag individual candidates whose overall profile suggest that they would be particularly valuable to the objectives of the workshop in ways that might not be fully captured by the selection procedure, and set aside a predefined number of available slots for candidates selected in this way. It is in the interest of the organizers to limit that number to a reasonably small fraction of the overall cohort, or the process will loose the benefits of the algorithmic procedure laid out in this manuscript. 

%In either case, the preselected candidates and their attributes should still be taken into account during the algorithmic step of the selection. Instead of initializing the algorithm's first step e.g.\ at random, one may initialize the algorithm with the combined attributes of the preselected candidates. If the preselected cohort is strongly biased compared with the targets, \entrofy\ will aim to rebalance the overall cohort in this step of the procedure. In this way, accepting stand-out candidates outright can be straightforwardly combined with an algorithmic approach to generate a cohort that reflects both the variance in terms of merit in the candidate set, but is also balanced with respect to the overall targets and objectives of the selection committee.
